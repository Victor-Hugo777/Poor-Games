...
export class GameBusiness {
    ...
    criarJogo = async (nomejogo: string, genero: string, datalancamento: string, paginajogo: number, preco: number): Promise<any> => {
        if (!nomejogo || !genero || !datalancamento || !paginajogo || preco === undefined) {
            throw new Error('Todos os campos (nomejogo, genero, dataLancamento, paginajogo, preco) são obrigatórios');
        }
        ...
        try {
            ...
            return await this.gameData.criarJogo(nomejogo, genero, datalancamento, paginajogo, preco);
        } catch (error) {
            throw new Error("Erro ao criar novo jogo");
        }
    }

    atualizarJogo = async (id: string, nomejogo: string, genero: string, datalancamento: string, paginajogo: number, preco: number): Promise<any> => {
        if (!nomejogo || !genero || !datalancamento || !paginajogo || preco === undefined) {
            throw new Error("Todos os campos são obrigatórios");
        }

        try {
            return await this.gameData.atualizarJogo(id, nomejogo, genero, datalancamento, paginajogo, preco);
        } catch (error) {
